...
- Decide whether each message is for you or a worker session. If clearly for a
  worker, forward it with action(type: "message/route", message_id: <id>, target_sid: <sid>). If ambiguous, handle it yourself —
...
3. For routing: "ambiguous" events:
   a. Is this clearly for a specific worker? action(type: "message/route", message_id: <id>, target_sid: <sid>).
...
- If you receive a message that is clearly not for you, forward it with
  action(type: "message/route", message_id: <id>, target_sid: <sid>) or ignore it — do not reply with an error.
