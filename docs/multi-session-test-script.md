...
1. **[S1]** already connected via `action(type: "session/start")`
2. **[Verify]** `{ sid: 1, sessions_active: 1 }`

### 1.2 Second Session Joins

1. **[S2]** `action(type: "session/start", name: "Scout")`
...
1. **[S1]** `action(type: "session/list")` → both sessions listed
2. **[S2]** `action(type: "session/list")` → same list, `active_sid` matches caller

### 1.5 Name Collision Guard

1. **[S2 or another client]** Try `action(type: "session/start", name: "Primary")`
...
> The intro message sent by `action(type: "session/start")` is owned by the new session. Replying to
...
1. **[S2]** `send(type: "text", text: "I'm session 2")`
...
1. **[S1]** `send(type: "text", text: "S1 says hello")`
2. **[S2]** `send(type: "text", text: "S2 says hello")`
...
3. **[S1]** `action(type: "message/route", message_id, target_sid: 2)`
...
1. **[S1]** `action(type: "session/close")`
...
1. Restart S1 if closed in Phase 3.5: **[S1]** `action(type: "session/start")`
2. Both S1 and S2 active

### 4.1 Send DM (Auto-Granted)

1. **[S2]** `send(type: "dm", target_sid: 1, text: "Hey S1, found something.")`
...
1. **[S1]** `send(type: "dm", target_sid: 2, text: "Thanks, S2.")`
2. **[Verify]** S2 receives the DM

### 4.3 Revoke on Close

1. **[S2]** `action(type: "session/close")`
2. **[Verify]** DM permissions involving S2 revoked
3. **[Debug]** DM revocation log

### 4.4 Manual DM Request (New Session)

1. **[S2]** `action(type: "session/start", name: "Scout")` → operator approves → auto-DM-grant
2. **[S2]** `send(type: "dm", target_sid: 1, text: "I'm back")` → works immediately
...
2. **[S3]** `action(type: "session/start", name: "Builder")` → operator approves
3. **[Verify]** 3 sessions active, each sees fellow sessions
4. **[Verify]** `action(type: "session/list")` from any session shows all 3
...
1. **[S1]** `send(type: "text", text: "S1 speaking")`
...
1. **[S2]** `action(type: "session/close")` while messages are in its queue
...
- [ ] Governor delegation via `action(type: "message/route")`
