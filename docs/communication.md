...
On timeout ({ timed_out: true }):
  send a brief notify ("Still here — are you there?") then call dequeue() again.
```
...
On timeout (`{ timed_out: true }`): send a brief `notify` ("Still here — are you there?") then call `dequeue` again.
