...
- [ ] `session/reconnect` is a separate action from `session/start`
- [ ] `session/start` no longer accepts `reconnect` flag
- [ ] NAME_CONFLICT response strongly pushes token recovery before reconnect
- [ ] Reconnect response includes diagnostic guidance (why it happened, save token NOW)
- [ ] Operator dialog is clean — approve/deny only, no explanation text
- [ ] Breadcrumb chain: every response hints to the next likely action
- [ ] All existing tests updated for the API split
