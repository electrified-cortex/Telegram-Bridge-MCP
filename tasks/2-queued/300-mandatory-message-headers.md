...
- [ ] All outbound `sendMessage` calls include session header when `activeSessionCount() >= 2`
- [ ] Header format: `🤖 {name}\n` prepended to message text
- [ ] No header when only 1 session is active
- [ ] Topic prefix and session header compose correctly (header first, then topic, then body)
- [ ] `editMessageText` includes header in multi-session
- [ ] File send captions include header in multi-session
- [ ] Voice sends skip header (or handle gracefully)
- [ ] `markdownToV2()` handles the header line without breaking
- [ ] Tests: single session → no header
- [ ] Tests: multi-session → header present
- [ ] Tests: header + topic prefix composition
- [ ] Tests: header in editMessageText
