...
**Worker scope (no push required):**
- Annotated local tags created at the correct merge commits for all missing v7.x versions (v7.0.0, v7.0.1, v7.1.0, v7.2.0, v7.2.1, v7.2.2, v7.3, v7.4, plus v7.5.x and v7.7.x–v7.11.x once hashes are provided).
- Each tag message includes the version label and the PR/commit it corresponds to.
- Tag creation is idempotent: re-running the task does not error if a tag already exists.
- Worker reports the full list of created tags for operator/Foreman review before push.

**Foreman/operator scope (not Worker AC):**
- Tags pushed to origin (requires push access; explicit Foreman or operator step after Worker report).
- Optional: GitHub releases created from tag annotations.

**Blocking prerequisite:**
- Curator must provide commit hashes for v7.3 (confirmed at 4747c989 in Notes but version string mismatch needs verification), v7.4 (fd635289), v7.5.x, and v7.7.x–v7.11.x before dispatch.
