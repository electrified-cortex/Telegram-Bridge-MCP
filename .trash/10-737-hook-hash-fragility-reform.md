...
## ⛔ CLOSED — premise stale (2026-06-21)

The `.agents/` PowerShell hook system this task targets no longer exists. `pretooluse-permissions.ps1` is only present in `-archive/`. Current hooks are `event.sh` (Claude Code session loggers only — no script gating). The SHA-256 allowlist friction problem described here is moot. No implementation needed.

Evidence: `find` confirms `pretooluse-permissions.ps1` only at `-archive/agents/.archive/*/hooks/` and `electrified-cortex` task-engine has no hook gating.
