...
1. **No acceptance criteria** — requirements listed but no binary, testable ACs.
2. **No YAML frontmatter fields** — `priority: PRI-1` is not numeric; missing `agent_type`, `model_class`, `reasoning_effort`.
3. **No delegation block** — no Executor/Reviewer.
4. **Prerequisite not confirmed** — "sub-session-presentation-cleanup-2026-05-22.md" listed as prerequisite but not tracked as queued; status unknown.

Fix: add binary ACs per requirement (R1-R7 each become testable AC), add frontmatter, delegation, confirm prerequisite status.
