# TMCP sub-session UX bugs — operator-observed during SpecAudit test, 2026-05-25

Captured from operator voice msg 61404 (mid-day PT) — observations during the first live test of sub-session-dispatch infrastructure with the SpecAudit child session.

## What worked

- Sub-agent dispatched and ran to completion (~20 min, 64 tool uses, 93k tokens).
- Operator could interact directly with the child session and steer it (issued an over-specification doctrine directive that the sub-agent implemented).
- Parent (Curator) was not polluted by the side conversation.
...
Operator framed these as TMCP-side bugs / feature misses — not Curator-side. Capturing for the TMCP queue.

1. **Sub-session should not announce itself as a separate session.** It "should just show the topic." Current behavior emits a `session_joined` service message with a name + SID announcement. For a single-sub-session-at-a-time pattern, the announcement creates the wrong mental model — operator is talking to "Curator on a topic," not to a separate identity.
...
3. **Name tag mismatch.** The child's name tag should be identical to the parent's, distinguished only by the topic suffix. Currently a separate name tag appears for SpecAudit. Operator's mental model: parent identity + topic, not parent + child.
...
- Operator said: "the fact that it's working at all is sweet, I'm impressed" — so functional path is intact.
- The UX/announcement bugs are cosmetic-but-mental-model-violating. They don't block the feature working, but they pollute operator's sense of who they're talking to.
...
- Out of scope for Curator. Route to TMCP queue (`electrified-cortex/Telegram-Bridge-MCP/tasks/`).
- Capture-only here; spec the fix when Curator is asked to work on TMCP improvements.
