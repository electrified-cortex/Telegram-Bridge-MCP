...
> Operator (distilled): delivering the orienting service message up front aligns the agent once in its context, after which it generally stops repeating the bad pattern.
...
8. **NEW** `onboarding_presence_signals` — presence cascade: react on receipt → show-typing for short work → **animation for work that exceeds show-typing's window** (~20s) or has no clear ETA. >30s silence with no escalation is a violation. Reference `help('presence')`.
   ...
   Operator stated rule (distilled): presence was identified too slowly; it could have been signalled via an eyes/processing reaction or a thinking animation. Show-typing alone is not enough when the work runs long — escalate to animation BEFORE show-typing's timeout expires.
...
- Operator asked (distilled) whether this was unclear from the service messages — it was clear; the agent missed it.
...
Third instance same session: started a release-branch task, set show-typing for 20s, work ran longer, no animation escalation. Operator (distilled): presence was identified too slowly; an eyes/processing reaction or a thinking animation would have served. Adds to the case for putting presence-cascade rules in the first-dequeue bundle.
...
Operator on this same gap (distilled): surprised no service message prompted the agent to signal the user sooner — is this a failure of the presence protocol or of how agents are guided to be present?
