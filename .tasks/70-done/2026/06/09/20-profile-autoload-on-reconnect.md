...
**Source:** Operator-reported via S-IM 2026-06-08 (Pilot wants this confirmed). Code-grounded by Curator ground-truth verification against `src/tools/session/start.ts`.
...
**Repro (operator):** operator pod did `session/reconnect` last night with an `autoload: true` profile; `reminder/list` came back empty until a manual `profile/load`.
...
- Small, well-bounded (few-line add + test). agent implements; Curator stages; **operator commits**.
