...
overseer_gate: "PASS 2026-06-23T00:09Z — Q1=health-check.ts:runHealthCheck(), Q2=DEQUEUE_GAP_GRACE_MS constant, Q3=clock continues on SSE drop if notification delivered"
...
## Overseer gate resolutions (2026-06-22)

**Q1 — File location:** `src/health-check.ts`, function `runHealthCheck()`, lines 238-293 (the 'newly unhealthy sessions' loop). Worker will also need `session-queue.ts` to access pending message count per session. NOTE: `isSseMonitorActive` guard at line 252 (imported from `src/tools/activity/file-state.ts`) already implements the SSE-active suppression — preserve it, do not refactor away.

**Q2 — Configurability:** No env var. Add a named constant `DEQUEUE_GAP_GRACE_MS = 600_000` (10 min) alongside `HEALTH_THRESHOLD_MS`. Follows the same pattern; can be exposed via env var as P3 later.

**Q3 — SSE drops before dequeue:** Clock continues if SSE notification was already delivered for the current pending message batch. Clock does not start if SSE dropped before any SSE notification was sent for that batch. Rationale: once the agent has been notified, they are responsible for dequeuing — SSE liveness after delivery does not reset the obligation.

**Implementation note:** AC2 (suppress when active SSE) is already implemented in the current codebase (line 252). Worker must verify it stays intact after refactor. The new work is: (1) add queue-empty guard (AC1), (2) add SSE-notification timestamp tracking per pending message (AC3/AC4), (3) wire the new 10-min grace window from first notification.

## Verification

**Verdict: APPROVED**
Overseer gate: PASS 2026-06-23T00:09Z
Incorporated in release/7.15.0 commit a2a2f30f. Build: 0 TS errors. Tests: 3907/3907 pass. All adversarial review items resolved.
