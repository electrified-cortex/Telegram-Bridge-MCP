...
**RESOLVED 2026-06-20 by the agent.** `tools/monitor.sh` was completely rewritten (the TMCP audit / canonical monitor migration). The new monitor.sh (148 lines) is a pure bash mtime-polling script — no `.signal` files, no `watch.ps1`, no self-reset logic. The inbox-monitor self-reset cycle cannot occur on the current codebase. Archived as resolved.
