...
Note on 20-2201: The "stable through compaction without re-arming" premise in 20-2201 mixes two concepts — (a) the watcher process survives, and (b) the Monitor TaskCreate subscription must always be re-registered. Current docs are correct: the watcher process check (File-B/C) avoids unnecessarily restarting the watcher, but `Monitor()` must always be re-run. 20-2201 as-written would produce a regression if "nothing at compaction recovery" were taken literally. The coordinating agent must clarify 20-2201's intent before it can proceed.
