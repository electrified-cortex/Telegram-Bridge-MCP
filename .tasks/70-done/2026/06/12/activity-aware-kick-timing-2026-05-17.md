...
1. Add new action `profile/kick-gate` — canonical knob going forward.
   **⚠️ Naming update 2026-06-12:** `profile/kick-lockout` is now occupied — it is a backward-compat alias for `profile/notify-debounce` (merged 376a431c). The activity-kick gate action must be named `profile/kick-gate` instead.
...
4. After one release cycle, `profile/kick-debounce` returns a structured error pointing to `profile/kick-gate`.

New module: `activity-kick-gate.ts` — do NOT use `kick-lockout.ts` (already gone, renamed to `notify-debounce.ts` in 376a431c).
...
- `profile/kick-debounce` deprecation response shape: `{ ok: true, deprecated: true, replacement: 'profile/kick-gate', translated_value: <legacy_value> }`.
...
- New module: `activity-kick-gate.ts` (not `kick-lockout.ts`).
