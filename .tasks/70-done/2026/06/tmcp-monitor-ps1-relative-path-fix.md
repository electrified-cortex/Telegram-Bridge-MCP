...
**Why:** TMCP onboarding text was written assuming the caller's CWD is the TMCP repo. This works on some pods because earlier sync-work copied `tools/monitor.sh` into that pod root (vendored). Curator's pod root has no such copy, so the relative path resolves to a missing file.
