...
When TMCP returns the `activity_file_monitor_instructions` service message, the script path is *relative* (`tools/monitor.ps1`, `tools/monitor.sh`). An agent environment that has no `tools/` directory in its working directory will hit exit 127 the first time it tries to arm the activity-file monitor.

**Why:** TMCP onboarding text was written assuming the caller's CWD is the TMCP repo. This works in environments where an earlier sync copied `tools/monitor.sh` into the agent's working directory (vendored). Environments without such a copy will see the relative path resolve to a missing file.
