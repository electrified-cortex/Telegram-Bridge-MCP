...
/**
 * Handle profile/notify-gate action.
 *
 * GET (ms omitted): return current per-session notify gate lockout window.
 * SET (ms provided): set per-session notify gate lockout window.
 *
 * The notify gate is a post-notify lockout: after the session is notified (activity
 * file touched / SSE stream notified), further notifications are suppressed until either
 ...
 */
export function handleNotifyGate({ token, ms }: { token: number; ms?: number }) {
  ...
  if (ms < NOTIFY_DEBOUNCE_MIN_MS || ms > NOTIFY_DEBOUNCE_MAX_MS) {
    return toError(
      `notify_gate ms must be between ${NOTIFY_DEBOUNCE_MIN_MS} and ${NOTIFY_DEBOUNCE_MAX_MS}. Got ${ms}.`,
    );
  }
