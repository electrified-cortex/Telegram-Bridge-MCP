...
const mocks = vi.hoisted(() => ({
  ...
  getNotifyDebounceMs: vi.fn((_sid: number) => 500),
...
vi.mock("./session-manager.js", () => ({
  ...
  getNotifyDebounceMs: (sid: number) => mocks.getNotifyDebounceMs(sid),
}));
...
describe("channel", () => {
  beforeEach(() => {
    ...
    mocks.getNotifyDebounceMs.mockReturnValue(500);
  ...
  describe("notifyChannelSubscriber", () => {
    ...
    it("fires again once the cooldown window has expired", async () => {
      ...
      mocks.getNotifyDebounceMs.mockReturnValue(-1000);
      const { server, notify } = makeServer();
      registerChannelSubscriber(SID, TOKEN, server);
      notifyChannelSubscriber(SID);
