  ...
  while (Date.now() < deadline) {
    ...
    for (const u of updates) {
      ...
      if (u.message.text.trim() === code) {
        ...
        console.log("");
        console.log(dim("    — VS Code (settings.json → mcp.servers):"));
        ...
        console.log(dim(`      "env": { "BOT_TOKEN": "${token.slice(0, 8)}…", "ALLOWED_USER_ID": "${userId}" }`));
        console.log(dim(`    }`));
        console.log("");
        console.log(dim("    — Claude Desktop (claude_desktop_config.json → mcpServers):"));
        console.log(dim('    "telegram": {'));
        console.log(dim(`      "command": "node",`));
        console.log(dim(`      "args": ["${resolve(__dirname, "..", "dist", "index.js").replace(/\\/g, "\\\\")}"],`));
        console.log(dim(`      "env": { "BOT_TOKEN": "${token.slice(0, 8)}…", "ALLOWED_USER_ID": "${userId}" }`));
        console.log(dim(`    }`));
        console.log("");
        console.log(dim("    — Claude Code (.mcp.json in project root → mcpServers):"));
        console.log(dim('    "telegram": {'));
        console.log(dim(`      "command": "node",`));
        console.log(dim(`      "args": ["${resolve(__dirname, "..", "dist", "index.js").replace(/\\/g, "\\\\")}"],`));
        console.log(dim(`      "env": { "BOT_TOKEN": "${token.slice(0, 8)}…", "ALLOWED_USER_ID": "${userId}" }`));
