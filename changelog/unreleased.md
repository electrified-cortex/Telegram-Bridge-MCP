...
- **`set_reaction` temporary mode** — `set_reaction` now accepts optional `restore_emoji` and `timeout_seconds` parameters; when either is present the reaction becomes temporary: it auto-reverts to `restore_emoji` (or removes itself) on the next outbound action or after the timeout; eliminates the 👀→🫡 manual two-step; the separate `set_temporary_reaction` tool was removed in favour of this unified API; `temp-reaction.ts` module manages the active slot and is cleared by the outbound proxy on every send/edit/file
